...
## Using useMonitor for Direct Access

The `useMonitor` hook provides direct access to a Monitor instance. This is useful when you need to read signals imperatively within a component:
...
<Callout type="warn">
  For most use cases, prefer `useComputed` over `useMonitor`. The `useComputed` hook provides memoization and custom equality options that `useMonitor` does not.
</Callout>
...
<Cards>
  <Card title="useComputed API" description="Complete API reference" href="/docs/api/use-computed" />
  <Card title="useMonitor API" description="Advanced monitor access" href="/docs/api/use-monitor" />
  <Card title="React Integration Concepts" description="How signals work with React" href="/docs/concepts/react-integration" />
  <Card title="Batching Updates" description="Preventing multiple re-renders" href="/docs/guides/batching" />
</Cards>
