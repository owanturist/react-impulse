...
The `useComputed{:dart}` hook reads signal values in React components and triggers re-renders when those values change.
...
Writable signals need stable storage (`useState{:dart}` or `useRef{:dart}`) to survive re-renders. Derived signals use `useMemo{:dart}` when their source may change.

See [Using Signals in React](/docs/guides/using-signals-in-react#creating-in-components) for detailed patterns and examples.

## Dependency Array Behavior [#dependency-arrays]

Without a dependency array, the compute function executes on every re-render. When a dependency array is provided, the compute function only re-executes when one of the listed dependencies changes (compared by reference).

<Callout>
  The `useComputed{:dart}` hook supports exhaustive dependency lint rules. See [Biome and ESLint](/docs/guides/biome-and-eslint) for setup instructions.
</Callout>
