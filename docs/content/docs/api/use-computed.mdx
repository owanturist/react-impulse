...
Writable signals need stable storage (`useState{:ts}` or `useRef{:ts}`) to survive re-renders. Derived signals use `useMemo{:ts}` when their source may change.
...
<Callout>
  The `useComputed{:ts}` hook supports exhaustive dependency lint rules. See [Biome and ESLint](/docs/guides/biome-and-eslint) for setup instructions.
</Callout>
