...
When `useB` is `true{:ts}`, the component depends on `a`, `b`, and `useB`. When `useB` becomes `false{:ts}`, the component only depends on `a` and `useB`.

### Render Optimization

The hook uses React's `useSyncExternalStore` internally, which ensures:

- Consistent reads during concurrent rendering
