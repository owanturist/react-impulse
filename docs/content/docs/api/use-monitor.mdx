...
The `useMonitor` hook provides direct access to a Monitor instance for reading signals in components.
...
The `useMonitor` hook is the simpler primitive — it returns a Monitor and leaves reading and rendering logic to you. The `useComputed` hook builds on top of it, adding memoization, dependency arrays, and custom equality checks.
