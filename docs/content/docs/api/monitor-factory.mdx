...
The `Monitor` type is opaque. You pass it to `signal.read(monitor){:ts}` to track dependencies.
...
1. **Memory management** - Always call the disconnect function returned by `connect(){:ts}` when the factory is no longer needed
