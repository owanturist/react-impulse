...
The `untracked{:dart}` function reads signal values without creating reactive dependencies.

## When to Use [#when-to-use]

Reads a signal's value without establishing a reactive dependency. Useful when you need a snapshot of the current value without subscribing to future changes — for example, in event handlers, async operations, or one-off computations.

See [Reading and Writing Signals](/docs/guides/reading-and-writing-signals#untracked) for common patterns.
