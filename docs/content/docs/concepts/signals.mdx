...
By default, signals use `Object.is` to determine if a value has changed. You can customize this:
...
// This won't trigger updates - values are equal
position.write({ x: 0, y: 0 })

// This will trigger updates
position.write({ x: 10, y: 0 })
```
...
The library provides several types for working with signals:

| Type | Description |
|------|-------------|
| `Signal<T>` | A signal that can be read and written |
| `ReadonlySignal<T>` | A signal without the `write` method (e.g., derived signals created without a setter) |
| `ReadableSignal<T>` | Interface for anything that can be read with a Monitor |
| `WritableSignal<T>` | Interface for anything that can be written to |

### Exposing Read-Only Views

A common pattern is to expose signals as read-only to prevent external code from modifying them:
...
// Controlled mutations only through functions
...
Signals can be cloned to create independent copies:
...
const derived = Signal((monitor) => writable.read(monitor) * 2)
