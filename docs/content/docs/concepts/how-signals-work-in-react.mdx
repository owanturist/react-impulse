...
Signals are plain objects -- pass them via props or context like any other value. The library has no built-in providers or store hooks:
...
The writable `user` signal is created once in `App` with `useState{:ts}`, and the derived `isLoggedIn` is created with `useMemo{:ts}`. Both are passed down via context. When `user` changes, only `UserProfile` and `LoginStatus` re-render -- `App` itself does not, because the context value (the signal references) stays stable.
