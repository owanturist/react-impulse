...
const fahrenheit = Signal(
  ...
  // Setter: when fahrenheit is written, update celsius
...
The setter function receives the new value and should update the source signals. The getter then recomputes based on the updated sources.
...
While signals are synchronous, you can model async states:
...
// Derived helpers
const isLoading = Signal((monitor) =>
  userState.read(monitor).status === "loading"
)
...
// More optimal - watchers can subscribe to just what they need
...
Components can then watch only the specific derived signal they care about.

### Use Custom Equality

When a derived signal produces objects or arrays, custom equality can prevent cascading updates:

```ts
const coords = Signal((monitor) => ({
