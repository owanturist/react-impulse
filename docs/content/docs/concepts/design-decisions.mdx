...
description: How signal APIs in Angular, Solid, Preact, Vue, Svelte, and Qwik inspired the design choices behind this library
---
...
A single `Signal(){:ts}` factory covers all three roles — writable, derived, and bidirectional derived. The first argument determines which kind you get: a plain value creates a writable signal, a function creates a derived one. Most other frameworks use separate functions (`signal{:ts}` + `computed{:ts}`, `createSignal{:ts}` + `createMemo{:ts}`) — two names for what is conceptually a single operation.
...
// Outside — read without tracking
console.log(untracked(count)) // 5
```
...
Every other framework uses implicit tracking — calling the getter or accessing a property automatically registers the dependency. This library makes tracking explicit through the `Monitor{:ts}` parameter — you can always trace a computation's dependencies. The tradeoff is extra syntax in exchange for explicit dependency tracking.
...
The `.write(){:ts}` method handles both direct values and updater functions, keeping reads and writes as deliberate method calls. Frameworks like Preact, Vue, and Qwik use property assignment (`.value = x{:ts}`), while Svelte makes reactivity look like plain JavaScript. Angular and Solid separate the setter into its own method or function.
...
All frameworks provide read-only derived signals. The syntax varies, but the goal is identical: declare a computation and the framework keeps it current. `Signal(){:ts}` is reused here — pass a function as the first argument to create a derived signal.
...
Angular, Solid, Preact, Svelte, and Qwik do not support bidirectional derived signals. Vue's `computed({ get, set }){:ts}` directly inspired this library's `Signal(getter, setter){:ts}` — both create two-way bindings without manual synchronization.
...
Effects run synchronously and return a dispose function, similar to Preact. Angular and Vue schedule effects as microtasks, Solid and Svelte run them after rendering, and Qwik uses explicit `track(){:ts}` calls inside `useTask$(){:ts}`. The synchronous approach is predictable — you always know when the effect has run.
...
Only Angular, Solid, and this library support custom equality. The default behavior across all frameworks is reference equality (`Object.is{:ts}`). Custom equality avoids unnecessary recomputations for data that is structurally identical but referentially different, like arrays or objects.
...
Both `untracked(fn){:ts}` for arbitrary code and `untracked(signal){:ts}` for a single signal are supported — inspired by Angular. Most frameworks offer some form of untracked reads, except Vue which has no direct equivalent. Preact uses `.peek(){:ts}` on individual signals instead of a global function — though the name doesn't convey "untracked", which can surprise developers who see it return a value without realizing it skips dependency tracking.

## What This Library Does Not Do

This library omits several features found in other signal libraries:

- **No compiler magic** — There is no build step that transforms variable access into reactive subscriptions. Svelte's `$state{:ts}` and `$derived{:ts}` runes eliminate boilerplate but require a compiler step. This library runs in any standard JavaScript environment without a build step.

- **No JSX auto-unwrapping** — Preact signals can update the DOM directly from JSX, bypassing component re-renders. This library requires `.read(){:ts}` calls inside components, though you can build thin wrapper components that accept a signal as a prop and subscribe to it — effectively achieving fine-grained updates scoped to a small subtree.
...
- **No implicit scheduling** — Angular and Vue batch updates implicitly through their change detection and async queue systems. This library requires explicit `batch(){:ts}` calls when you need to group multiple writes.

- **Immutable data by default** — Signals compare values by reference equality. Mutating an object in place and passing the same reference to `.write(){:ts}` is a no-op unless you set `equals: () => false{:ts}`. This matches React's model — `useState{:ts}`, `useMemo{:ts}`, props, and context all assume immutability — so signals integrate naturally into React's update model.
