...
A single `Signal(){:ts}` factory covers all three roles — writable, derived, and bidirectional derived. The first argument determines which kind you get: a plain value creates a writable signal, a function creates a derived one. Most other frameworks use separate functions (`signal{:dart}` + `computed{:dart}`, `createSignal{:dart}` + `createMemo{:dart}`) — two names for what is conceptually a single operation.
...
Every other framework uses implicit tracking — calling the getter or accessing a property automatically registers the dependency. This library makes tracking explicit through the `Monitor{:dart}` parameter — you can always trace a computation's dependencies. The tradeoff is extra syntax in exchange for explicit dependency tracking.
...
- **No compiler magic** — There is no build step that transforms variable access into reactive subscriptions. Svelte's `$state{:dart}` and `$derived{:dart}` runes eliminate boilerplate but require a compiler step. This library runs in any standard JavaScript environment without a build step.
...
- **Immutable data by default** — Signals compare values by reference equality. Mutating an object in place and passing the same reference to `.write(){:ts}` is a no-op unless you set `equals: () => false{:ts}`. This matches React's model — `useState{:dart}`, `useMemo{:dart}`, props, and context all assume immutability — so signals integrate naturally into React's update model.
