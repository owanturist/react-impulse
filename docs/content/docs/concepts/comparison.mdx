...
- **`@owanturist/signal`** — Fine-grained reactive signals. State lives outside React in plain signal instances. Dependencies are tracked automatically — a `Monitor{:dart}` handle is passed at the call site to opt each read into tracking.

- **Redux** — A single immutable store with unidirectional data flow. State changes happen through dispatched actions processed by reducers. The indirection is intentional — it creates a predictable, auditable update path.
  <Callout>
  All Redux examples on this page use Redux Toolkit (`@reduxjs/toolkit`), the official, recommended way to write Redux logic.
  </Callout>
...
```tsx tab="Module scope"
import { Signal } from "@owanturist/signal"

// Accessible anywhere via import
...
```tsx tab="Component scope"
import { Signal } from "@owanturist/signal"
import { useState } from "react"

function Counter() {
  // Owned by this component instance — not shared
  const [count] = useState(() => Signal(0)) // [!code highlight]
  return <CounterDisplay count={count} />
}
```
...
const store = configureStore({ // [!code highlight]
...
const useCounter = create<{ // [!code highlight]
  count: number;
  increment: () => void
}>((set) => ({
  count: 0,
...
const countAtom = atom(0) // [!code highlight]
...
function Counter() {
  const actorRef = useActorRef(counterMachine) // [!code highlight]
  const count = useSelector(actorRef, (snapshot) => snapshot.context.count)
  return <span>{count}</span>
}

// Shared scope — createActorContext provides a Provider
const CounterContext = createActorContext(counterMachine) // [!code highlight]
```

Redux is the only library that _requires_ a `<Provider>{:dart}` — components cannot access the store without it. Zustand and Jotai work without providers; their hooks connect to module-level stores directly. XState offers both patterns: `useActorRef(){:ts}` for component-local actors and `createActorContext(){:ts}` for shared access via a Provider. `@owanturist/signal` is the most flexible — signals are plain values that work as module globals, component state via `useState{:dart}`, props, or context, with no library infrastructure at all.
...
function CounterBadge({ count }: { count: ReadableSignal<number> }) {
  const value = useComputed(count) // [!code highlight]
...
function CounterBadge() {
  const count = useSelector((state: RootState) => state.counter.count) // [!code highlight]
  return <span>{count}</span>
}
```
...
function CounterBadge() {
  const count = useCounter((state) => state.count) // [!code highlight]
  return <span>{count}</span>
}
```
...
function CounterBadge() {
  const count = useAtomValue(countAtom) // [!code highlight]
  return <span>{count}</span>
}
```

```tsx tab="XState — shared"
import { CounterContext } from "./counterContext"

// Descendants use context hooks — no props needed
function CounterBadge() {
  const count = CounterContext.useSelector((snapshot) => snapshot.context.count) // [!code highlight]
  return <span>{count}</span>
}
```

```tsx tab="XState — local"
import { useSelector, type ActorRefFrom } from "@xstate/react"
import type { counterMachine } from "./counterMachine"

// actorRef passed explicitly as a prop
function CounterBadge({ actorRef }: { actorRef: ActorRefFrom<typeof counterMachine> }) {
  const count = useSelector(actorRef, (snapshot) => snapshot.context.count) // [!code highlight]
  return <span>{count}</span>
}
```

Redux, Zustand, and Jotai give every component implicit access to shared state — any descendant can reach into the store without receiving anything via props. XState works the same way with `createActorContext(){:ts}`, or you can pass `actorRef{:dart}` explicitly as a prop. `@owanturist/signal` uses explicit passing via props or context — the component tree shows the data flow. The key insight: passing a signal as a prop without reading it creates no subscription and no re-render. Intermediate components are transparent pass-throughs.
...
- **Jotai** — `<Provider>{:dart}` creates an isolated atom scope where atoms have independent values. Useful for testing, modals, and repeated widgets that each need their own state. Atoms resolve to the nearest Provider; without one, they use the global store.
...
## Defining and Using State [#defining-state]
...
function Counter() {
  const monitor = useMonitor() // [!code highlight]
  const value = count.read(monitor) // [!code highlight]
...
function Counter() {
  const value = useSelector((state: RootState) => state.counter.count) // [!code highlight]
...
function Counter() {
  const value = useCounter((state) => state.count) // [!code highlight]
...
function Counter() {
  const value = useAtomValue(countAtom) // [!code highlight]
...
function Counter() {
  ...
  const value = useSelector(actorRef, (snapshot) => snapshot.context.count) // [!code highlight]
...
Every library ends up in a React component, but the setup cost varies significantly. Redux requires the most ceremony — slices, reducers, a store, typed hooks, and a `<Provider>{:dart}` — before a component can read or write a value; XState demands a similar investment in machine definitions and event types. Zustand collapses state and actions into a single `create(){:ts}` call with no provider required, while Jotai and `@owanturist/signal` are the most concise — one function call creates a reactive value. All five libraries subscribe via hooks, not React context: Redux, Zustand, and XState use selector functions; Jotai selects at the atom level; `@owanturist/signal` offers two hooks — `useMonitor(){:ts}` returns a monitor for reading signals directly in the render body, and `useComputed(){:ts}` accepts a signal reference or factory function for memoized derivations.
...
function FullName() {
  ...
  const full = useComputed((monitor) => { // [!code highlight]
...
function FirstNameOnly() {
  ...
  const first = useComputed(firstName) // [!code highlight]
...
function FullName() {
  ...
  const full = useSelector( // [!code highlight]
...
function FirstNameOnly() {
  ...
  const first = useSelector((state: RootState) => state.user.firstName) // [!code highlight]
  return <span>{first}</span>
}
```
...
function FullName() {
  ...
  const full = useUserStore( // [!code highlight]
...
function FirstNameOnly() {
  const first = useUserStore((state) => state.firstName) // [!code highlight]
  return <span>{first}</span>
}
```
...
function FullName() {
  ...
  const full = useAtomValue(fullNameAtom) // [!code highlight]
  return <span>{full}</span>
}

function FirstNameOnly() {
  // Subscribes to firstNameAtom only
  const first = useAtomValue(firstNameAtom) // [!code highlight]
  return <span>{first}</span>
}
```
...
function FullName({ actorRef }: { actorRef: ActorRefFrom<typeof userMachine> }) {
  const full = useSelector( // [!code highlight]
    actorRef,
    (snapshot) => `${snapshot.context.firstName} ${snapshot.context.lastName}`,
  )
  return <span>{full}</span>
}

function FirstNameOnly({ actorRef }: { actorRef: ActorRefFrom<typeof userMachine> }) {
  const first = useSelector(actorRef, (snapshot) => snapshot.context.firstName) // [!code highlight]
  return <span>{first}</span>
}
```
...
- **`@owanturist/signal`** — The Monitor tracks exactly which signals each `useComputed(){:ts}` call reads. The compute function only re-runs when one of those specific signals changes — writes to unrelated signals are invisible. Dependencies are dynamic: if a conditional branch skips reading a signal, that signal won't cause re-renders. On top of that, each signal can define a custom `equals{:dart}` option that prevents propagation at the source — if a write produces a value equal to the current one, no subscribers are notified at all. `useComputed(){:ts}` then applies its own `equals{:dart}` as a second gate: if the compute result hasn't changed, no re-render is enqueued.
...
const count = Signal(0)
const doubled = Signal((monitor) => count.read(monitor) * 2) // [!code highlight]
const quadrupled = Signal((monitor) => doubled.read(monitor) * 2) // [!code highlight]
```
...
const selectDoubled = createSelector(selectCount, (count) => count * 2) // [!code highlight]
const selectQuadrupled = createSelector(selectDoubled, (doubled) => doubled * 2) // [!code highlight]
```

```ts tab="Zustand"
// Derived values are typically computed in selectors
function Component() {
  const doubled = useCounter((state) => state.count * 2) // [!code highlight]
  const quadrupled = useCounter((state) => state.count * 4) // [!code highlight]
}
```
...
const countAtom = atom(0)
const doubledAtom = atom((get) => get(countAtom) * 2) // [!code highlight]
const quadrupledAtom = atom((get) => get(doubledAtom) * 2) // [!code highlight]
```

```ts tab="XState"
// XState has no built-in derived state.
// Use selector functions on state snapshots:
const selectDoubled = (state) => state.context.count * 2 // [!code highlight]
const selectQuadrupled = (state) => state.context.count * 4 // [!code highlight]
```
...
Zustand offers a lean middleware system — `persist{:dart}` for localStorage, `devtools{:dart}` for Redux DevTools integration, `immer{:dart}` for mutable update syntax. It strikes a balance between simplicity and extensibility.

Jotai provides `jotai-devtools`, Suspense integration, and utilities like `atomWithStorage(){:ts}` for persistence. Its atom model integrates well with React's concurrent features.

XState offers Stately Studio — a visual editor for designing state machines — alongside `@xstate/inspect` for runtime visualization and `@xstate/test` for model-based testing.

`@owanturist/signal` has a minimal ecosystem. There are no devtools, no middleware system, and no persistence utilities. `@owanturist/signal-form` provides form state management. The library focuses on a small, correct core rather than breadth.
...
const count = Signal(0)               // WritableSignal<number> // [!code highlight]
...
type RootState = ReturnType<typeof store.getState> // [!code highlight]
type AppDispatch = typeof store.dispatch // [!code highlight]
...
const useStore = create<{ // [!code highlight]
  name: string
  age: number
...
const nameAtom = atom("Alice")              // PrimitiveAtom<string> // [!code highlight]
...
const userMachine = setup({
  types: {
    context: {} as { name: string; age: number }, // [!code highlight]
    events: {} as
...
## TODO
- [ ] Should research and document a section about managing dynamic parts of the state. For example, a list of items where you want to add/remove items and have each item be reactive. This is a common use case that is natural to implement efficiently with signals - just nest Signals into signals like `Signal<Array<{name: Signal<string>, age: Signal<number>}>>` - the items (objects with signals) are created dynamically and added to the `Signal<Array>`, and it would be helpful to compare how different libraries handle it. In contrast, redux has only one global store, and those values so those items would be also global. I guess the whole point boils down to whether it is possible to have nested states: nested redux stores, jotai atoms/or whatever, xstate actors, zustand stores, and how they compare to nested signals. This is a common use case that is not well covered in the documentation of these libraries, and it would be helpful to have a section about it in the comparison. THere is something similar in [framework comparison](./design-decisions.mdx) in "Nested Signals" section - something similar should be here, because this is one of the key strengths of signals - the ability to have nested reactive values that can be created dynamically and added/removed from the parent signal, and then read the values of such data structures via regular API just exactly as none-nested signals are read.