...
### `Impulse(initialValue, options?){:ts}` [toc] [#impulse-factory]

<p id="impulse-factory" />
...
The configuration object for the `Impulse`.
...
The `compare` function determines when the `Impulse` considers values different.<br/>
...
### `Impulse(){:ts}` [toc] [#impulse-factory-optional]

<p id="impulse-factory-optional" />
