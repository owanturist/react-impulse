...
/**
 * A hook reads an {@link impulse} value whenever it updates but enqueues a re-render only when the resulting value is different from the previous.
 *
 * @template TValue the type of the Impulse value.
 *
 * @param impulse anything that implements the {@link ReadableImpulse} interface.
 *
 * @returns the {@link impulse}'s value.
 *
 * @version 1.0.0
 */
function useComputed<TValue>(impulse: ReadableImpulse<TValue>): TValue

/**
 * A hook that executes the {@link compute} function whenever any of the involved Impulses' values update but enqueues a re-render only when the resulting value is different from the previous.
 *
 * @template TResult the type of the {@link compute} result.
 *
 * @param compute a function that provides {@link Monitor} as the first argument and subscribes to all Impulses calling the {@link Impulse.read} method inside the function.
...
function useComputed<TResult>(
  computeOrReadableImpulse: ((monitor: Monitor) => TResult) | ReadableImpulse<TResult>,
  dependencies?: DependencyList,
  options?: UseComputedOptions<TResult>,
): TResult {
  const transform = useCallback(
    (monitor: Monitor) => {
      if (isFunction(computeOrReadableImpulse)) {
        return computeOrReadableImpulse(monitor)
      }

      return computeOrReadableImpulse.read(monitor)
    },
    // biome-ignore lint/correctness/useExhaustiveDependencies: pass dependencies as is or factory is the only dependency
    dependencies ?? [computeOrReadableImpulse],
  )
