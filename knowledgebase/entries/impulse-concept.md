...
- **`initialValue`** (`T`): The initial value for the Impulse.
...
Returns a new Impulse with an initial `T` value.
...
Creates a new Impulse with an initial value of `undefined` but type of `undefined | T`.
...
See [compare-concept](./compare-concept.md) for details on the compare function used in ImpulseOptions.
